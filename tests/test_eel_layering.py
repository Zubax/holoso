...
# `_pe/_ops.py` is the partial evaluator's sole HIR-facing module: it builds constants and conversions and folds through
# the operators' own `evaluate` (the one-answer mandate), while the rest of `_pe` stays HIR-free; `_lower.py` composes
# the stages and returns the lowered `Hir` alongside the printed Eel of each pass.
