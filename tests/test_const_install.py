...
installs is driven by a microcode const-pool selector (``mc_ccidx``) muxing among its constants, rather than the bare
...
def test_multi_distinct_const_install_emits_selector() -> None:
    ...
    assert "mc_ccidx_" in generate_verilog(lir).verilog


@pytest.mark.cosim
@pytest.mark.parametrize("sim", SIMULATORS)
def test_multi_distinct_const_install_cosim(sim: str) -> None:
    """RTL == model for a register installing two distinct constants: the ``mc_ccidx`` selector must pick the right one."""
