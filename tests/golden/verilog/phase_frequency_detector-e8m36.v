...
module phase_frequency_detector (
    ...
    input  wire in_ref_edge,
    input  wire in_fb_edge,
    input  wire in_clear,
    // OUTPUT/STATE PORTS
    // Valid when out_valid is pulsed. Publicly visible states are included here.
    output wire state_up,
    output wire state_down,
    // DIAGNOSTIC PORTS
...
always @(posedge clk)
    case (ucode_addr_q)
        0: ucode_q <= 7'h05;  // land b0=b0|b3, b1=b1|b4
        1: ucode_q <= 7'h10;  // land b3=b0&b1
        2: ucode_q <= 7'h0a;  // land b0=~b3&b0, b1=~b3&b1
        3: ucode_q <= 7'h60;  // land b3=~b2&b0, b4=~b2&b1
        4: ucode_q <= 7'h00;
        5: ucode_q <= 7'h00;
        6: ucode_q <= 7'h00;
...
wire [ 1:0] uc_op_b0 = {2{transacting}} & ucode_word[0 +: 2];
wire [ 1:0] uc_op_b1 = {2{transacting}} & ucode_word[2 +: 2];
wire [ 1:0] uc_op_b3 = {2{transacting}} & ucode_word[4 +: 2];
wire        uc_op_b4 = transacting & ucode_word[6 +: 1];
...
always @(posedge clk) begin
    ...
    if (in_ready && in_valid) bregs[0] <= in_ref_edge;
    else case (uc_op_b0)
        2'd1: bregs[0] <= bregs[0] | bregs[3];
        2'd2: bregs[0] <= ~bregs[3] & bregs[0];
    endcase
    if (in_ready && in_valid) bregs[1] <= in_fb_edge;
    else case (uc_op_b1)
        2'd1: bregs[1] <= bregs[1] | bregs[4];
        2'd2: bregs[1] <= ~bregs[3] & bregs[1];
    endcase
    if (in_ready && in_valid) bregs[2] <= in_clear;
    ...
    if (rst) begin
        ...
        err_pc_q      <= 0;
        transacting_q <= 0;
        bregs[3] <= 1'b0;  // _ref_pending reset snapshot
        bregs[4] <= 1'b0;  // _fb_pending reset snapshot
    end else begin
        pc <= next_pc;
        ...
        case (uc_op_b3)
            2'd1: bregs[3] <= bregs[0] & bregs[1];
            2'd2: bregs[3] <= ~bregs[2] & bregs[0];
        endcase
        if (uc_op_b4) bregs[4] <= ~bregs[2] & bregs[1];
    end
end
...
assign state_up = bregs[3];
assign state_down = bregs[4];
