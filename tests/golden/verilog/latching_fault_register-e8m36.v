...
module latching_fault_register (
    // CONTROL PORTS
    ...
    input  wire in_overcurrent,
    input  wire in_overvoltage,
    input  wire in_overtemp,
    // OUTPUT/STATE PORTS
...
localparam           CYCW      =   2;  // err_pc width: enough for any executing step (0..present)
...
localparam [PCW-1:0] PRESENT   =   3;  // executing step on which the outputs are valid in the array
localparam [PCW-1:0] LASTPC    =   5;  // = PRESENT + FETCH_LAG; out_valid asserts here
...
always @(posedge clk)
    case (ucode_addr_q)
        0: ucode_q <= 5'h07;  // land b3=b3|b0, b4=b4|b1, b5=b5|b2
        1: ucode_q <= 5'h08;  // land b0=b3|b4
        2: ucode_q <= 5'h10;  // land b0=b0|b5
        3: ucode_q <= 5'h00;
        4: ucode_q <= 5'h00;
        5: ucode_q <= 5'h00;
...
wire        uc_op_b3 = transacting & ucode_word[0 +: 1];
wire        uc_op_b4 = transacting & ucode_word[1 +: 1];
wire        uc_op_b5 = transacting & ucode_word[2 +: 1];
wire [ 1:0] uc_op_b0 = {2{transacting}} & ucode_word[3 +: 2];
...
always @(posedge clk) begin
    ...
    if (in_ready && in_valid) bregs[0] <= in_overcurrent;
    else case (uc_op_b0)
        2'd1: bregs[0] <= bregs[3] | bregs[4];
        2'd2: bregs[0] <= bregs[0] | bregs[5];
    endcase
    if (in_ready && in_valid) bregs[1] <= in_overvoltage;
    if (in_ready && in_valid) bregs[2] <= in_overtemp;
    ...
    if (rst) begin
        ...
        err_pc_q      <= 0;
        transacting_q <= 0;
        bregs[3] <= 1'b0;  // _overcurrent reset snapshot
        bregs[4] <= 1'b0;  // _overvoltage reset snapshot
        bregs[5] <= 1'b0;  // _overtemp reset snapshot
    end else begin
        pc <= next_pc;
        ...
        if (uc_op_b3) bregs[3] <= bregs[3] | bregs[0];
        if (uc_op_b4) bregs[4] <= bregs[4] | bregs[1];
        if (uc_op_b5) bregs[5] <= bregs[5] | bregs[2];
    end
end
