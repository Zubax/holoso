...
the cosim-invisible hazard the gate exists to stop. The idle length k and fetch_lag come from the environment so the
test can sweep k and stay valid as fetch_lag is configurable.
"""
...
async def state_inert_during_dwell(dut: Any) -> None:
    # A cycle-0 write to a persistent-state slot sits on `ucode[0]`. While the PC dwells idle at pc 0 the held word
    # re-fetches every cycle, so without the `transacting` gate on the write opcode it would commit to the state
    # register before any transaction.
    k = int(os.environ["HOLOSO_DWELL_K"])
    ...
    for idle in range(k + 1):
        ...
        assert got == reset_bits, (
            ...
            f"reset {reset_bits:#x} -- a write committed on the held ucode[0]"
        )
        await RisingEdge(dut.clk)
        await FallingEdge(dut.clk)
