...
#
# TWO EDGES BACK for `datapath_sem` and `is_bool_fact`, and this is the M2 trade again in miniature: emission
# used to DEFINE both rules itself, as `_fact_sem` and `_is_bool_fact`. Which kind a fact takes in the datapath
# is what nine of emission's refusals were decided on, so the settlement that now makes those refusals has to
# read the same rule -- and two spellings that agreed on today's corpus would be free to disagree about which
# kernels refuse. The rule therefore moved into the shared fact vocabulary and both phases import it. The meter
# reads that as +2 debt; what actually happened is that emission stopped owning a decision.
...
def test_emission_rejection_sites_only_shrink() -> None:
    ...
    #
    # NINE OF THE SIXTEEN THEN MOVED into `settle_use_sites`, the resolution's last act: the constant-carrier and
    # NaN refusals, the non-datapath materialization, the two unlowerable-operator refusals, the three power
    # refusals and the two boolean-comparison refusals. They moved as ONE GROUP because they are ORDER-COUPLED --
    # measured on four kernels whose earlier op materializes an unrepresentable constant and whose later op is an
    # unexpandable power, which report the constant today and would silently re-attribute to the power if only
    # the power check moved. Two more retired into proved assertions: an exhaustive 351-kernel operator
    # enumeration reaches the integer lowering with exactly ten of the thirteen operators, all handled, and
    # `SemType` being closed at three makes the cast fallback unreachable by exhaustion.
    #
    # THE FIVE THAT STAY ALL CONSUME `type_of`, and that is now the whole of the remaining debt rather than a
    # claim about it. Three are the scalar materializer's kind mismatches, where carried-kind drift is MEASURED
    # rather than argued: 117 instances over 558 kernels of a fact reading integer while the node is float-
    # carried, arriving through the SSA at a state boundary. The other two are the return-kind checks, which see
    # drift of their own (4 instances) -- but theirs is emission's own choice of constant materialization, so a
    # settlement could reproduce it only by mirroring that choice, and the message would still read "returns
    # float" for a kernel returning a Known integer.
    #
    # The corroboration is the frozen corpus, regenerated byte-identical, plus a 573-kernel differential against
    # the pre-move tree in which every rendered message, location and accepted HIR node count is unchanged and
    # only the internal class differs: `legacy_beyond_carrier_constant` and `legacy_power_chain` join
    # `legacy_never_returns` in being raised by the resolver, and all three render as `UnsupportedConstruct`.
    assert measured == (5, 5, 5, 9, 73), (
        f"emission's refusal shape is {measured}, recorded (5, 5, 5, 9, 73) as "
