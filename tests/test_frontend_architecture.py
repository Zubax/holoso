...
def test_emission_rejection_sites_only_shrink() -> None:
    ...
    #
    # THE 16 THAT STAYED ARE NOT DEBT THIS STEP DECLINED TO PAY. Fourteen are operand refusals, and each reads
    # `type_of(vid)` on a node emission has already built -- the carried kind can differ from the fact's, since
    # an integer crossing a state boundary is float-carried while its fact still reads integer, so no resolver
    # over final facts can decide them without rebuilding the SSA. The other two are the last leg of the return
    # contract for the same reason: what the returned value was EMITTED as. Moving those needs emission's typing
    # to become a settled table of its own, which is a step in its own right and not this one.
    #
