...
from ._modelref import DEFAULT_IFMT, SHIPPED_TUNING, build_lir, default_ops
...
def _measure(name: str) -> Metrics:
    lir: Lir = build_lir(
        lower_to_mir(optimize(lower(_EXAMPLES[name]()).hir), default_ops(_FMT), _FMT, DEFAULT_IFMT),
        name,
        SHIPPED_TUNING,
    )
    straight = (
