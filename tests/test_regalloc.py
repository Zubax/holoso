...
def test_the_build_carries_the_residue_into_the_binding() -> None:
    ...
    a = builder.input("a", FloatType(_FMT))
    b = builder.input("b", FloatType(_FMT))
    c = builder.input("c", FloatType(_FMT))
    ...
    builder.output("out_0", builder.operation(fadd, [second, early], signs))
    builder.ret()
    mir = builder.finish()
