...
(construct, layout and bankalloc all import from here) to keep those stage modules acyclic.
"""
...
class BlockSchedules:
    """
    Every block scheduled once per graph, with the cross-block overlap threaded through: the per-block schedule;
    the values each block receives past an overlapping predecessor's terminator, mapped to their block-local landing
    cycle (fed to the allocator's liveness so a spilled register stays reserved in the block, and identical to the
    scheduler's `livein_landing` so the two cannot drift); the terminator offset of every OVERLAPPING
