...
The spike ran on branch `spike/resolved-ir`, base `4f1dd4c`, tip `dc76fbf` (branch deleted after this record was
committed; its append-only evidence ledger was `docs/decisions/spike-ledger.md` on that branch, entries E1-E10,
and is summarized here because the branch does not survive).
...
| SC2 mechanical emitter | FAILS as literally specified; the substantive half holds — see below |
...
| SC4 size | FAIL: residualizer 1275 LOC against the 1200 subset bound, after three behavior-preserving size passes (1584 → 1373 → 1281 → 1275). Extrapolation passed (1403 ≤ 1800) |
...
The criterion is, as written, unsatisfiable by any emitter that emits HIR: HIR's own constant folding refuses,
so every HIR-building module reaches the rejection type. That makes this a defect in the criterion as much as
in the prototype. The substantive property SC2 existed to test DOES hold, and is what the prototype actually
measured: the emitter's closure reaches no Fact, no registry, no `Py*` op, no callback, and no frontend
decision module, the emitter's own source contains zero `raise` statements and zero lambdas, and every
emitter-owned refusal moved upstream with diagnostic parity (the negative packet). Stage 4's M0 guard must be
written to that property — a frontend-decision-layer ban — rather than to a transitive `_errors` ban that
nothing can satisfy.
...
SC1's byte identity is established against the spike's base `4f1dd4c`, NOT against `freeze-1`, which is 35
...
The materialized resolved spine CLOSED. Every semantic decision across all 8 witnesses fit the frozen
one-`Def`/three-RHS/`Route` vocabulary with ZERO schema growth after the freeze, and a 351-line mechanical
emitter reproduced production HIR and Verilog byte-for-byte from resolved data alone. The residualizer performs
zero transfer replays, zero live host reads, and zero registry resolutions — it consumes `ResidualUnit` fields
only, so recording is evidence-atomic from the adapter down. Projected net production change for the emitter
half is roughly +358 LOC gross with `_emit.py` (1500 LOC, 42 emission-decided refusal sites) deleted.
...
landing that consumes the pre-authorized canonical gate. More decisively: the prototype INHERITS
`executable_edges` and `block_in` from the analyzer. Those are exactly the sets the deferral seam corrupts —
the baseline's documented silent miscompiles are stale-reachability defects — so a spine materialized over
them would bake the defect into the new boundary instead of dissolving it. The resolved spine must RECOMPUTE
reachability and typing from the stabilized facts. An early cutover would materialize the known-defective
seam; the schema, the verifier, and a dormant A/B harness may land earlier, but the functional cutover follows
M1-M6.
